...
insert into person(name, phone, age, grade)
...
select name, phone, age, grade, task, difficulty, answer, topic, studentsbook, page, website
from person p
	left join person_to_task pt on p.id = pt.person_id
	left join task t on t.id = pt.task_id
	left join topic tc on tc.id = t.topic_id;
